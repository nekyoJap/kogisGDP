...
| データベース | Aurora MySQL / DynamoDB | - | CQRSにより書き込み/読み込みで分離 |
| トランザクション管理 | Spring Transaction | - | 宣言的トランザクション（@Transactional） |
...
二重計上・二重請求・二重決済を防止するための規範を以下に定める。
...
| 原則 | 説明 |
|------|------|
| 冪等キー必須 | 金銭・残高・ポイントに影響する更新処理には冪等キーを必須としなければならない |
| ユニーク制約 | 業務キー（取引ID、決済ID等）に対してデータベースレベルのユニーク制約を設定しなければならない |
| 処理状態チェック | 処理実行前に既存の処理状態を確認し、重複実行を防止しなければならない |
| トランザクション内検証 | 冪等性チェックとデータ更新は同一トランザクション内で実行しなければならない |
...
| パターン | 適用場面 | 実現方法 |
|---------|---------|---------|
| 冪等キー記録方式 | API経由の更新処理 | リクエストの冪等キーを専用テーブルに記録し、重複時は既存結果を返却 |
| ユニーク制約方式 | 新規登録処理 | 業務キーに対するユニーク制約で重複INSERTを防止 |
| 状態遷移方式 | 状態を持つ処理 | 現在の状態を確認し、不正な状態遷移を拒否 |
| バージョン比較方式 | 更新処理 | バージョン番号による楽観ロックで競合を検出 |
...
| 処理種別 | 防止方法 | 必須要件 |
|---------|---------|---------|
| 決済承認（オーソリ） | 承認番号＋取引IDのユニーク制約 | 必須 |
| 売上確定 | 売上ID＋承認番号のユニーク制約、処理状態チェック | 必須 |
| 入金処理 | 入金IDのユニーク制約、入金済みフラグチェック | 必須 |
| ポイント付与 | 付与ID＋対象取引IDのユニーク制約 | 必須 |
| 請求計上 | 請求ID＋明細IDのユニーク制約 | 必須 |
...
| 原則 | 説明 |
|------|------|
| 削除禁止 | データの物理削除ではなく、取消レコードまたは取消フラグで表現しなければならない |
| 取消ID管理 | 取消処理には一意の取消IDを付与し、二重取消を防止しなければならない |
| 元取引参照 | 取消・訂正レコードは元取引への参照を必ず持たなければならない |
| 状態整合性 | 取消対象の現在状態を確認し、取消可能な状態でのみ処理を実行しなければならない |
...
|------|------|---------|--------|
| 1.0 | 2026-01-14 | 初版作成 | - |
| 1.1 | 2026-01-15 | 規範用語定義追加、技術スタック追加、関連文書追加、二重処理防止規範追加 | - |
