...
| メッセージング | 未確定（Kafka/RabbitMQ/AWS SQS等） | - | 別途技術選定 |
| イベント仕様形式 | AsyncAPI | 2.0 | イベント定義に使用 |
| JSON処理 | Jackson | - | イベントペイロードのシリアライズ |
...
| 関連ドキュメント | 関係性 |
|------------------|--------|
| ソフトウェアアーキテクチャ方針書 | DDD/クリーンアーキテクチャ・Outbox標準部品規約を参照 |
| トランザクション方針書 | トランザクション境界・整合性方針を参照 |
| APIガバナンス方針書 | 同期API・Event APIの設計方針を参照 |
| データモデリング方針書 | Outboxテーブル・Saga状態テーブルの設計を参照 |
...
| 原則 | 説明 |
|------|------|
| 後方互換性 | 既存の消費者が新しいスキーマのイベントを処理できなければならない |
| 前方互換性 | 新しい消費者が古いスキーマのイベントを処理できることが望ましい |
| 破壊的変更禁止 | フィールド削除、型変更、必須フィールド追加は禁止する |

#### 8.4.2 許可される変更と禁止される変更

**表23-3 — スキーマ変更可否**

| 変更種別 | 可否 | 理由・条件 |
|---------|------|-----------|
| 任意フィールドの追加 | 許可 | 後方互換性を維持 |
| 必須フィールドの追加 | **禁止** | 既存消費者が処理できなくなる |
| フィールドの削除 | **禁止** | 依存する消費者に影響 |
| フィールド型の変更 | **禁止** | デシリアライズ失敗の原因 |
| フィールド名の変更 | **禁止** | 実質的にフィールド削除＋追加 |
| デフォルト値の追加 | 許可 | 後方互換性を維持 |
| enumへの値追加 | 条件付き許可 | 消費者が未知の値を処理できる場合のみ |

### 8.5 イベントペイロードの機微データ規範

#### 8.5.1 禁止データの定義

**表23-4 — イベントに含めてはいけないデータ**

| 分類 | データ例 | 扱い |
|------|---------|------|
| 禁止データ | カード番号（PAN）、CVV、暗証番号 | イベントペイロードに含めてはならない |
| 要マスキング | 氏名、住所、電話番号、メールアドレス | イベントペイロードに含めてはならない（参照IDで代替） |
| 要暗号化 | 口座番号、生年月日 | 含める場合は暗号化必須 |
| 一般データ | 契約ID、取引ID、金額、ステータス | 含めてよい |
...
| 原則 | 説明 |
|------|------|
| 機微データ排除 | イベントには機微データ本体を含めず、参照IDのみを含めなければならない |
| 必要時取得 | 機微データが必要な場合は、参照IDを使用してAPIで取得しなければならない |
| 最小限原則 | イベントには処理に必要な最小限のデータのみを含めなければならない |
...
| 発信側BC/システム | 受信側BC/外部 | 連携内容 | 方式種別 | Saga有無 | 備考 |
|----------------|-------------|---------|---------|---------|------|
| 売上確定BC（例） | ポイントBC（例） | 売上確定通知 | Outbox | 無 | 単純な通知のみ |
| 審査BC（例） | 発行BC（例） | 審査完了→発行指示 | Saga | 有 | 申込フロー全体をSagaで管理 |
| 請求BC（例） | 会計連携BC（例） | 請求確定通知 | Outbox | 無 | 会計システムへの連携 |
...
|------|------|---------|--------|
| 1.0 | 2026-01-15 | 初版作成 | - |
| 1.1 | 2026-01-15 | 規範用語定義追加、技術スタック追加、関連文書追加、イベントスキーマ進化方針追加、機微データ規範追加、取消・訂正イベント原則追加 | - |
