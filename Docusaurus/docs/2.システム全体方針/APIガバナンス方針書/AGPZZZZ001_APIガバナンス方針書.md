...
| 7.1 | API企画書のテンプレート・必須項目 | テンプレート策定が必要 |
| 7.2 | バージョン表現方式 | プロジェクト標準の策定が必要 |
| 7.2 | 旧バージョン並行提供期間 | 運用ルールの策定が必要 |
| 7.2 | 廃止告知期間 | 運用ルールの策定が必要 |
| 8.1 | API仕様形式の採用形式 | 技術選定が必要 |
| 8.1 | レビュー担当ロール | 体制定義が必要 |
| 8.2 | ベースパス方針の具体値 | 設計標準の策定が必要 |
| 10.1 | スロットリング／レートリミットの具体値 | 性能要件との整合が必要 |
| 10.2 | 分散トレーシングの適用有無 | 技術選定が必要 |
