...
| バックエンドフレームワーク | Spring Boot | 3.x | Spring Data JPA使用 |
| 書き込みDB | Aurora MySQL | 3.x | CQRS書き込み側 |
| 読み込みDB | Aurora MySQL / DynamoDB | - | CQRS読み込み側（用途により選択） |
| ORM | JPA/Hibernate | - | エンティティマッピング |
...
| APIガバナンス方針書 | APIスキーマとデータモデルの整合性を参照 |
| 非同期連携方針書 | Outboxテーブル・Saga状態テーブルの設計を参照 |
...
|------|------|---------|--------|
| 1.0 | 2026-01-15 | 初版作成 | - |
| 1.1 | 2026-01-15 | 規範用語定義追加、技術スタック追加、関連文書追加 | - |
