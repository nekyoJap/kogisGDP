...
以下の【形式】に基づいて、対象システムの「ソフトウェアアーキテクチャ方針書」を作成してください。\
...
本書で定義されたアーキテクチャ方針に反する設計・実装は行ってはならない。
...
#### 4.2.1 境界づけられたコンテキスト（Bounded Context）
| 原則 | 説明 |
|-----|------|
...
| 原則 | 説明 |
|-----|------|
| 整合性境界 | 集約はトランザクション整合性の境界を形成する。集約内の整合性はトランザクションで保証する |
| 集約ルート | 集約へのアクセスは必ず集約ルートを通じて行わなければならない |
| 小さな集約 | 集約は可能な限り小さく設計し、パフォーマンスとスケーラビリティを確保する |
...
| 原則 | 説明 |
|-----|------|
| 識別子 | エンティティは一意な識別子を持ち、識別子で同一性を判定する |
| ライフサイクル | エンティティは生成・更新・削除のライフサイクルを持つ |
| 振る舞い | エンティティは自身の状態を変更する振る舞いを持ち、ドメインロジックをカプセル化する |
...
| 等価性 | 値オブジェクトは属性の組み合わせで等価性を判定する |
| 自己完結 | 値オブジェクトは自身の妥当性検証ロジックを持つ |
| 副作用なし | 値オブジェクトのメソッドは副作用を持たず、新しい値オブジェクトを返す |
| ユビキタス言語 | 値オブジェクトはユビキタス言語の語彙を表現する |

### 6.4 ドメインサービス設計の原則

| 原則 | 説明 |
|-----|------|
| ステートレス | ドメインサービスは状態を持たない |
| エンティティ・VOに属さないロジック | 複数の集約にまたがる、または特定のエンティティ・VOに属さないドメインロジックを配置する |
| ドメイン層配置 | ドメインサービスはドメイン層に配置する（アプリケーションサービスとは異なる） |
| インタフェース定義 | 外部依存がある場合は、ドメイン層でインタフェースを定義する |
...
| 原則 | 説明 |
|-----|------|
| ドメインモデル使用 | 更新系はドメインモデル（集約）を通じて処理する |
| 整合性担保 | トランザクション整合性を集約単位で担保する |
| イベント発行 | 状態変更時にドメインイベントを発行し、クエリモデルの更新をトリガーする |
...
| 結果整合性 | コマンドモデルとの整合性は結果整合性で実現する |
| プロジェクション | ドメインイベントを購読し、クエリモデル（プロジェクション）を更新する |
...
| 検討観点 | 説明 |
|---------|------|
| 監査要件 | すべての状態変化を追跡する必要がある場合に検討 |
| 複雑性 | イベントソーシングの複雑性とメリットを比較検討 |
| 既存資産 | 既存のデータモデル・運用との整合性を考慮 |

---

## 第8章：SBE/Living Documentation方針
...
| 区分 | 定義形式 | 管理単位 | 説明 |
|-----|---------|---------|------|
| 振る舞い | Gherkin（.feature） | BC単位 | ユースケース・ビジネスルールの振る舞いを具体例で定義 |
| 構造 | 仕様書（Markdown等） | BC単位 | エンティティ・VO・集約の構造・属性を定義 |

### 8.3 Gherkinによる振る舞い定義

#### 8.3.1 Gherkin記述方針
| 原則 | 説明 |
|-----|------|
| ユビキタス言語 | Gherkinのシナリオはユビキタス言語で記述しなければならない |
| ビジネス視点 | 技術的な実装詳細ではなく、ビジネスの視点でシナリオを記述する |
| 具体例 | 抽象的なルールではなく、具体的な例（値）を用いて記述する |
| 独立性 | 各シナリオは他のシナリオに依存せず、独立して実行可能でなければならない |

#### 8.3.2 Gherkinファイルの管理
| 項目 | 方針 |
|-----|------|
| 配置場所 | BC単位のディレクトリに配置する |
| 命名規則 | フィーチャー名は対応するユースケース・ドメインルールと整合させる |
| バージョン管理 | ソースコードと同一リポジトリで管理し、変更履歴を追跡可能にする |
...
| 対象 | 定義内容 |
|-----|---------|
| 集約 | 集約ルート、構成要素、整合性ルール |
| エンティティ | 識別子、属性、状態遷移 |
| 値オブジェクト | 属性、妥当性検証ルール、等価性ルール |
| ドメインイベント | イベント名、属性、発生条件 |

#### 8.4.2 構造仕様と振る舞い仕様の紐付け
| 原則 | 説明 |
|-----|------|
| 相互参照 | 構造仕様書とGherkinシナリオは相互に参照可能でなければならない |
| 一貫性 | 構造仕様で定義したエンティティ・VOは、Gherkinシナリオで同じ名称を使用する |
| 変更同期 | 構造変更時はGherkinシナリオへの影響を確認し、必要に応じて更新する |

### 8.5 Living Documentationの原則
...
| 継続的実行 | CI/CDパイプラインでGherkinテストを継続的に実行する |

#### 8.5.2 仕様変更時のルール
| ルール | 説明 |
|-------|------|
| Gherkin先行 | ビジネスルール変更時は、まずGherkinシナリオを更新する |
| テスト失敗の受容 | Gherkin更新後、対応するテストが失敗することを確認する |
| 実装による修正 | テストが通るように実装を修正する（TDDサイクル） |
| 仕様書同期 | 構造に影響がある場合は、構造仕様書も同時に更新する |
...
| 保証項目 | 方法 |
|---------|------|
| 振る舞いの同期 | Gherkinテストの継続的実行により、振る舞い仕様とコードの同期を保証 |
| 構造の同期 | 構造仕様からの再生成と差分検出により、構造仕様とコードの同期を検証 |
| 乖離の検出 | CI/CDでの自動検証により、仕様とコードの乖離を早期に検出 |
